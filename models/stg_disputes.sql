{{ config(
    materialized='table',
    description='Cleaned and deduplicated dispute data. Removes CDC duplicates and validates dispute amounts.',
    meta={
...
with source_data as (
    select
        ...
        _ab_cdc_deleted_at,
        row_number() over (partition by dispute_id order by _airbyte_extracted_at desc) as rn
...
deduplicated as (
    select
        ...
        updated_at,
        _airbyte_extracted_at,
        cast(created_at as date) as dispute_date,
        case
